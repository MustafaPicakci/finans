...
- `apps/server/prices.ts`: `fetchTefasSnapshot(neededCodes)` — 5 fon türünü sırayla çeker (aralarında küçük bekleme), her sayfada görülen tüm fonları fırsatçı şekilde saklar. `RAPIDAPI_KEY` yoksa boş döner (elle girişe düşer). Opsiyonel `RAPIDAPI_KEY_2`: birinci anahtar 429 (kota/hız sınırı) alırsa aynı istek ikinci anahtarla tekrar denenir, kalan tüm istekler de ikinciyle devam eder (kullanıcının önerisiyle eklendi).
- **"Hepsini çek" varsayımı yanlış çıktı, erken çıkışla düzeltildi**: İlk tasarımda "API zaten tüm listeyi döndürüyor, hepsini saklayalım" denendi (kullanıcının önerisiyle) — ama gerçek veride tek bir fon türü (Menkul Kıymet) 8500+ satır/35 sayfa çıktı. Sayfa başına 250 kayıtla "hepsini çek" yaklaşımı ilk türü bile bitiremeden iki RapidAPI anahtarını da tüketti. Düzeltme: `fetchTefasSnapshot` artık aranan fon kodlarının (`neededCodes` — tutulan FON sembolleri) **tamamını bulduğu an** sayfalamayı/tür taramasını durduruyor; yol boyunca görülen diğer fonlar (aynı sayfanın bedavaya gelen verisi) yine de saklanıyor, sadece "daha fazlasını görmek için ekstra istek atma" davranışı durduruldu.
...
### Doğrulama
`pnpm build` temiz, 57 engine testi yeşil. Gerçek `.env` ile uçtan uca test edildi: ilk denemede 3 gerçek fon fiyatı başarıyla geldi (MAC, AFA, KHA), throttle mantığı doğrulandı (aynı gün tekrar tazeleme API'ye hiç gitmeden önbellekten döndü). "Hepsini çek" tasarımıyla yapılan ikinci canlı testte gerçek satır sayıları (fundType=1: 8500+ satır/35 sayfa) gözlemlenip iki RapidAPI anahtarı da tükendi — bu bulgu erken-çıkış tasarımına yol açtı (yukarı bkz.). Erken-çıkış değişikliği `pnpm build` ile tip kontrolünden geçti, henüz canlı doğrulanmadı (kota tükendi). **Bekleyen**: kota sıfırlanınca erken-çıkışın gerçekten az istekle (aranan 5-6 fonu bulup) tamamlandığını doğrulamak, daha önce boş kalan TP2/PHE/TLY'nin de gelip gelmediğini teyit etmek.
