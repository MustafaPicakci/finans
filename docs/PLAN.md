...
- ✅ **Faz 27 tamamlandı (Ağustos 2026)** — portföy grafiği yeniden yazıldı: (1) kâr/katkı ayrışması — değer eğrisinin altında "yatırdığın para" çizgisi, dönem özeti Δdeğer'i Δkâr + Δkatkı'ya böler; (2) varlık aç/kapa çipleri + `Değer`/`Getiri %` modu (portföy TWR, semboller saf fiyat getirisi); (3) Yahoo'dan 2 yıllık geriye doldurma (`backfillPriceHistory`, BIST/ETF/KRIPTO/DOVIZ; FON ve ALTIN kapsam dışı); (4) referans endeksler (BIST 100 / S&P 500 / NASDAQ / gram altın / dolar), hepsi o günün kuruyla TL'ye çevrilmiş, ayrı `benchmark_history` tablosunda. Kapsamı eksik günler çizilmez (`coveredOnly`) — fon fiyatı bilinmeyen günler sahte sıçrama üretirdi.
