...
### Doğrulama
`pnpm build` temiz, 57 engine testi yeşil. ETF ve dürüst-başarısızlık-raporlama gerçek veride uçtan uca doğrulandı. `returns-by-date` tasarımı henüz **tam** canlı doğrulanamadı: RapidAPI'nin "günlük" kotası Türkiye takvim gününe göre değil (muhtemelen UTC ya da 24 saatlik kayan pencere) sıfırlandığından, ertesi gün tekrar denendiğinde iki anahtar da hâlâ 429 veriyordu — ama bu durumda sistem artık doğru şekilde `ok:false` raporluyor (bayat veriyi gizlemiyor), bu da ayrıca doğrulanmış oldu. **Bekleyen**: kota gerçekten sıfırlandığında `returns-by-date`'in tek istekte tüm FON'ları (TP2/PHE/TLY dahil) getirip getirmediğini teyit etmek.
