...
- **Geçmişi geriye doldurma:** Fiyat geçmişi normalde yalnız uygulama çalıştığı günden itibaren birikir. `POST /api/prices/backfill` (gövde: `{"range":"2y"}`) tuttuğun **BIST / ETF / kripto / döviz** sembollerinin ve referans endekslerin 2 yıllık günlük geçmişini Yahoo'dan bir kerede doldurur. **TEFAS fonları ve altın kapsam dışıdır** (kaynakları geçmiş vermiyor), bu yüzden portföy *toplamı* yine kısa kalır — uzayan şey tek tek varlık serileri ve referans karşılaştırmasıdır.
