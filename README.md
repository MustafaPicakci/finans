...
- **Piyasa tarihleri (aynı takvimde):** Nakit takviminin üstünde ikinci bir katman var: tuttuğun şirketlerin **bilanço günleri**, geçmiş **bedelsiz/temettü** ex-date'leri ve **TCMB/Fed faiz kararları + TÜİK enflasyon** günleri. Takvim hücresinde küçük turuncu bir **kare** olarak görünür (planlı hareketin yuvarlak noktasından ayrı), tıklayınca günün panelinde "piyasa" bölümü açılır; liste görünümünde de hareketlerin arasına girer. Üstteki **piyasa** düğmesiyle kapatılır — kapattığın hâli hatırlanır. Ekranın asıl işi değişmez: hücredeki rakam hâlâ etkin nakittir, gün rengi hâlâ bakiyeden gelir. **`~` işaretli tarih tahminidir**: şirket bilanço gününü duyurmadıysa geçen yılın tarihinden türetiliyor, TÜFE günü de takvim kuralından (ayın 3'ü) geliyor — kesin tarihmiş gibi gösterilmiyor. Bedelsiz/temettü satırları **geçmiş** olaylardır; hiçbir kaynak bunları ileriye dönük vermediğinden beklediğin bir temettüyü Plan sekmesine tek seferlik gelir olarak yazman gerekir. TCMB/Fed takvimi elle bakımlıdır (yılda ~30 tarih, ikisi de yıl başından duyuruyor) — bitmeye yaklaşınca ekran kendisi uyarır.
...
- **Portföy sekmesi iki seviyelidir:** Açılışta **liste** gelir — portföyün toplam değeri, dönem kârı ve küçük bir seyir grafiği (sparkline), altında **varlık sınıfı dağılım şeridi** (ör. *KRIPTO %57 · ETF %19 · FON %17*), sonra portföy grupların ve **açık pozisyonların**. Burada analiz yoktur; ekran tek soruya cevap verir: *elimde ne var, ne kazandırıyor*. Bir portföye (ya da **Tüm portföy**e) tıklayınca **detay** ekranı açılır: son üç ayın getirisi ve toplam getiri, performans grafiği, varlık dağılımı ve üç sekme (*Mevcut Varlıklar*, *Varlık Takibi*, *İşlem Geçmişi*). **‹ Tüm Portföyler** ile listeye dönülür.
- **Portföy:** Ağırlıklı ortalama maliyet, her pozisyon kendi para biriminde. Satışta gerçekleşen K/Z = adet × (satış − ortalama) − komisyon. Pozisyon tamamen kapanıp yeniden açılırsa maliyet doğru sıfırlanır. **Tamamen sattığın bir varlık açık pozisyon listesinde görünmez** (eskiden "0 adet" satırı olarak duruyordu ve "elimde var mı?" sorusunu bulandırıyordu); geçmişi kaybolmaz — giriş/çıkış tarihi ve gerçekleşen K/Z ile **Varlık Takibi**'nde durur, gerçekleşen K/Z de toplamlara girmeye devam eder. İşlemi girerken **opsiyonel olarak bir nakit hesap** seçebilirsin: seçersen ALIŞ o hesaptan düşer, SATIŞ ekler (kayıt silinince geri alınır); seçmezsen portföy ve nakit ayrık kalır, bakiyeyi sen güncellersin.
...
- **Performans grafiği:** Portföy **detay** ekranında; **1H / 1A / 3A / 6A / 1Y / Tümü** düğmeleriyle istediğin pencerede görünür ve **her düğmenin altında o dönemin getirisi yazar** (ör. *1A +%6,8*), böylece hangi dönemde ne olduğunu tek tek tıklamadan görürsün. Detayda grafik **Getiri %** modunda açılır — oradaki soru "ne kadar param var" değil "nasıl performans gösterdim"dir; `Değer` moduna düğmeyle geçilir. Grafik **seçili portföy grubuna** kapsamlıdır. Dürüst kısıt: fiyat geçmişi günde bir anlık görüntü tuttuğundan en küçük çözünürlük **gündür** — aralık düğmeleri pencereyi daraltır, gün içi veri üretmez; fiyat çekmeye başlamadan önceki günler grafikte hiç yoktur (geriye dönük uydurma veri yok).
- **Varlık dağılımı (treemap):** Detaydaki kutucuk haritasında **kutunun boyutu ağırlık, rengi performanstır** (yeşil kazandırdı, kırmızı kaybettirdi); **Günlük / Toplam** arasında geçiş yaparsın. Fiyatı bilinmeyen varlık nötr gri kalır — sıfırmış gibi yeşile boyamak "bugün değişmedi" demek olurdu.
- **Mevcut Varlıklar tablosu:** Detaydaki ilk sekme; sembol başına **adet, maliyet, güncel fiyat, değer, ağırlık** ve **Günlük / Haftalık / Aylık / 3 Aylık / Yıllık / Toplam** getiri. Başlığa tıklayınca o kolona göre sıralanır. **İki farklı getiri aynı satırdadır ve karıştırılmamalı:** dönem kolonları **sembolün fiyat getirisidir** (piyasa ne yaptı), **Toplam** ise **senin** pozisyonunun ortalama maliyetine göre açık K/Z oranıdır. "Hisse %40 yükselmiş ama bende %5" bir çelişki değil, geç girmiş olmanın ta kendisidir. Verisi olmayan pencere **"—"** gösterir: TEFAS fonları ve altın geriye doldurulamadığından uzun pencereleri gerçekten boştur, eldeki en eski fiyatı taban saymak sahte bir getiri üretirdi.
- **Varlık Takibi (pozisyon dönemleri):** "Bu varlık portföye ne zaman girdi, ne zaman çıktı, sonuçta ne oldu?" sorusunun cevabı. Doğru birim sembol değil **pozisyon dönemidir**: Ocak'ta alıp Mart'ta tamamen sattığın ve Temmuz'da yeniden aldığın bir hisse **iki ayrı satırdır** — ayrı giriş/çıkış tarihi, ayrı maliyet, ayrı sonuç (kapanışta ortalama maliyet zaten sıfırlanır, tek satırda birleştirmek ikisini de gizlerdi). *Varlıklar* görünümü her dönemi ömrüyle (açılış → kapanış, konan/çıkan para, gerçekleşen + açık K/Z), *Dönem akışı* görünümü ise aynı olaylara zaman ekseninden bakar: o ay hangi varlık **açıldı / artırıldı / azaltıldı / kapandı** ve **net ne kadar para** portföye kondu. Net akış, grafikteki "yatırdığın para" çizgisiyle aynı hesaptır.
- **Son aylar:** Detay başlığındaki kutu son üç ayın ve tüm dönemin getirisini verir; rakamlar **TWR**'dır, yani ayın ortasında para eklemek getiriyi şişirmez.
- **Hareketler (işlem geçmişi):** Detayın son sekmesi. Her alış/satış **iki satırda** görünür: üstte yön rozeti, sembol ve tutar; altta tarih, adet × fiyat ve satışlarda **gerçekleşen K/Z** (bu rakam listeyi tararken görünmeli). Satıra dokununca işlemin **pozisyona etkisi** açılır: adet 10 → 20, ortalama maliyet ₺100 → ₺150, komisyon ve **portföy grubu seçici**. Sembol, yön (alış/satış/temettü/bedelsiz) ve dönem (3 ay / 6 ay / 1 yıl / tümü) filtrelenebilir; üstteki özet seçili aralık için toplam alış, satış, komisyon ve gerçekleşen K/Z'yi verir (TRY ve USD ayrı toplanır — işlem anındaki kur bilinmediğinden karıştırmak yanıltıcı olurdu). Bir pozisyona tıklayıp **hareketlerini gör** dersen yalnız o varlığın hareketleri süzülür. Filtreler yalnız *görünen* satırları kısar; ortalama maliyet her zaman tüm geçmişten hesaplanır.
- **Portföy grupları:** Varlıklarını mantıksal kaplara ayırabilirsin (ör. *Alfa Portföy*, *Emeklilik*, *Büyüme*). Gruplama **işlem düzeyindedir**: her alış/satış bir portföye (ya da hiçbirine — *Gruplanmamış*) aittir, bu yüzden **aynı sembolü iki portföyde ayrı ortalama maliyetle** tutabilirsin (farklı kurum/strateji). Portföy listesinde bir gruba tıklayınca **o grubun detay ekranı** açılır: pozisyonlar, performans, dağılım ve işlem geçmişi yalnız o grup için hesaplanır; **Tüm portföy** birleşik görünümdür. Mevcut bir işlemi, İşlem Geçmişi'nde satırı açıp içindeki menüden başka gruba taşıyabilirsin (yalnız gruplama değişir, tutar/bakiye etkilenmez). Grubu silmek işlemleri silmez — *Gruplanmamış*'a döner. **Net varlık, alokasyon ve nakit projeksiyonu gruplardan etkilenmez**; bu tamamen takip/raporlama katmanıdır.
- **Gelir/gider girişi (tek "+ Ekle" akışı):** Tarih bugün veya geçmişse *gerçekleşen* kayıt olur — bir hesap seçtiysen o hesabın bakiyesi anında değişir (gider düşer, gelir artar; kayıt silinince geri alınır), kategori seçtiysen gelir/gider defterine girer. Hesap seçmezsen kayıt yalnızca deftere girer, bakiyeye dokunmaz. Tarih ileriyse *plan* kalemi olur ve nakit projeksiyonuna girer; günü gelince Plan'daki **Gerçekleşti** ile tek tıkla deftere geçirip bakiyeye işlersin.
...
- **Kayıt düzenleme:** Gerçekleşen gelir/gider (Kayıtlar), plan kalemi (Plan), kart harcaması (Kartlar) ve portföy işlemi (Portföy → Hareketler) satırlarındaki **✎** ile düzenlenir; form "+ Ekle"dekinin aynısıdır, önden dolu gelir. Hesaba bağlı bir kaydı düzenlersen **bakiye otomatik düzeltilir**: eski tutarın etkisi eski hesaptan geri alınır, yenisi yeni hesaba işlenir — hepsi tek atomik işlemde, yani hesabı da değiştirebilirsin. Düzenleme kaydın türünü değiştirmez: gerçekleşen kayıt gerçekleşen kalır, plan kalemi plan kalemi (plan → defter geçişi Plan'daki "Gerçekleşti" düğmesidir). Tanım kayıtları (hesap, kart, kredi, kategori, düzenli kalem) ve vadeli mevduat hâlâ sil + yeniden ekle ile değişir — düzenli kalemin tutarı için "Değiştir" akışı vardır.
- **Uzun listeler parça parça gösterilir:** Kart harcamaları, kayıtlar, hesap hareketleri, transferler, portföy işlemleri ve pozisyon dönemleri gibi zamanla büyüyen listeler ilk açılışta bir dilim gösterir; altta **kaç kaydın gizlendiği yazar** ("50 / 81 işlem gösteriliyor") ve **Daha eski…** ile açılır, **başa dön** ile toplanır. Liste tek dilime sığıyorsa bu denetim hiç görünmez. Sessizce kesilen liste yok: bir şey gizleniyorsa ekran bunu söyler. Dönem özetleri (alış/satış/K/Z toplamları) **görünen dilime değil süzgecin tamamına** bakar — aksi halde "daha fazla göster"e her basışta toplamlar büyür, hangi rakamın doğru olduğu belirsizleşirdi.
...
## AI Asistan (Faz 22; sohbetler Faz 34'te sunucuya taşındı)
...
**Sohbetlerin hesabında durur (Faz 34).** Konuşmalar sunucuda saklanır, tarayıcıda değil: telefonda başlattığın bir sohbeti bilgisayardan sürdürürsün, uygulamayı silip yeniden kursan da kaybolmaz. Sekme **sohbet listesiyle** açılır: en son konuştuğun sohbet en üstte (açılış tarihine göre değil — dün başlayıp bugün devam ettiğin sohbet en üste gelir), her satırda başlık, zaman, mesaj sayısı ve varsa geri alınabilir işlem sayısı. Bir satıra dokunup kaldığın yerden devam edersin ya da **+ Yeni sohbet** ile temiz bir sayfa açarsın; sohbetin içindeyken **☰** listeye döner, **+** yeni sohbet başlatır. Hiç sohbetin yoksa liste yerine doğrudan boş sohbet gelir. **Başlığı değiştirebilirsin**: sohbetin üstündeki ada (yanındaki ✎ kalem işaretli) dokun, yaz, Enter'a bas (Esc vazgeçer, boş bırakırsan eski ad kalır). Başlık ilk cümlenden türetilir — "dün markete 1.250 TL harcadım, Axess'le" gibi — ve çoğu zaman kendi vereceğin ad daha iyidir. Yeniden adlandırmak sohbeti listede yukarı taşımaz; sıra son konuştuğun zamana göredir. Bir sohbetin içindeyken başka sekmeye bakıp dönersen listeye değil **kaldığın sohbete** düşersin; sayfayı yenilediğinde açılış yine listedir. Paylaşılan her harcama SMS'i kendi sohbetini açar — önceki konuşmanın bağlamı yeni bir olaya karışmasın diye.

Otomatik silme **yoktur**: sohbetin ancak sen silersen gider (listedeki ✕). Silmek yalnız yazışmayı siler, asistanın oluşturduğu kayıtlar defterde kalır — ama o sohbetteki işlemlerin **↩ Geri al** düğmesi bir daha görünmez, onay kutusu bunu açıkça yazar. İndirdiğin veri paketine (Hesabım → verilerini indir) sohbetler de dâhildir.

**Geri al.** Uygulanan işlemlerin dökümü sohbete bir mesaj olarak yazılır ("`✓ Gider: Market · 137,50 ₺`") ve **↩ Geri al** düğmesi tam o mesajın altında durur — olayın geçtiği yerde. Ayrıca sohbetin en altında **"Geri alınabilir işlemler"** listesi vardır: o sohbette yapılmış ve hâlâ geri alınabilen her işlem tek satırda, aramadan. İkisi aynı şeyi yapar ama farklı soruya cevap verir — liste "neyi geri alabilirim", mesaj "ne olmuştu". Geri alınan işlem listeden düşer, mesaj ise "geri alındı" yazmaya devam eder. (Faz 33'e kadar bu liste sekmenin altında ve sohbetten bağımsızdı; yalnız son 5 planı gösterdiği için daha eski bir planı geri almak imkânsızdı.) Geri alma, asistanın o planda *yarattığı* kayıtları ters sırada siler (önce işlem, sonra onu tutan hesap). Düzenleme, silme ve mutabakat geri alınamaz — eski hâlleri saklanmıyor, o yüzden onları düğme hiç önermez. Zaten var olan bir kaydı ("bu ekstre zaten ödenmişti") asistan yaratmadığı için ona da dokunmaz.

**Onayın bağlayıcıdır (Faz 34).** Onay kartındaki işlemler sunucuda saklanır ve **Onayla ve uygula** yalnız *plan kimliğini* gönderir: uygulanan şey, gördüğün şeyin ta kendisidir (eskiden uygulanacak işlemleri tarayıcı geri gönderiyordu). Kart sayfayı yenilesen de yerinde durur; 24 saat sonra "süresi doldu" der, çünkü bayat bir plandaki tutarlar artık gerçeği anlatmayabilir. Bir plan **yalnız bir kez** uygulanabilir: ağ tekrarı ya da ikinci tıklama 409 alır, aynı harcama iki kez yazılmaz.
