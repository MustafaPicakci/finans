...
export const TENANT_TABLES = [
  "accounts", "recurring", "loans", "oneoffs", "trades", "cards", "card_txs", "categories", "transactions", "deposits", "recurring_realized", "statement_payments", "recurring_amounts", "portfolios", "account_entries", "transfers",
] as const;
...
CREATE TABLE IF NOT EXISTS accounts (
  ...
  kind text NOT NULL DEFAULT 'banka',
  last_recon_date text,
...
/* Faz 9 — kimlik tutardan ayrıldı: tutar recurring_amounts zaman çizelgesinde yaşar.
   Eski DB'lerdeki amount kolonu initDb sonundaki in-place migration ile taşınıp düşürülür. */
...
/* Faz 11 — portföy grupları: kullanıcının varlıklarını mantıksal olarak ayırdığı kaplar
   ("Alfa Portföy", "Emeklilik", "Büyüme"). Gruplama İŞLEM düzeyindedir (trades.portfolio_id):
   aynı sembol iki portföyde ayrı pozisyon olarak durabilir (farklı kurum/strateji).
   Grupsuz işlemler NULL kalır ("Gruplanmamış"); portföy silinince işlemler kaybolmaz, NULL'a düşer. */
...
/* Faz 9 — düzenli kalemin tutar zaman çizelgesi: YM ayında geçerli tutar = from_month <= YM olan en
   büyük from_month'lu satır. '0000-01' sentinel'i = "baştan" (PK kolonu NULL olamaz). user_id
   TENANT_TABLES ALTER döngüsüyle eklenir. */
CREATE TABLE IF NOT EXISTS recurring_amounts (
  recurring_id integer NOT NULL REFERENCES recurring(id) ON DELETE CASCADE,
  from_month text NOT NULL,
  amount double precision NOT NULL,
  PRIMARY KEY (recurring_id, from_month)
);
...
/* Faz 15 — hesap hareket defteri. Bakiyeyi oynatan HER şey buraya bir satır yazar (aynı transaction
   içinde), böylece accounts.balance artık açıklanabilir bir sayıdır: değişmez kural
   balance = toplam(account_entries.amount) (açılış bakiyesi de bir satırdır: kind='acilis').
   kind hareketin kaynağını anlatır (islem/portfoy/mevduat/duzeltme/acilis); source_table+source_id
   kaydı geri almak/düzenlemek için kullanılır — geri alma eski tutarı yeniden HESAPLAMAZ, yazılmış
   satırı okuyup tersini uygular (düzenle/sil yollarındaki kayma sınıfı böyle kapanır). */
...
/* Faz 22 — asistanın UYGULADIĞI işlemlerin günlüğü ("geri al" için). Her satır bir onaylanmış
   işlemdir; undo_method/undo_path doluysa tek istekle geri alınabilir (kaydın silinmesi),
   boşsa geri alınamaz (düzenleme/silme/mutabakat — eski hâl saklanmıyor).
   TENANT_TABLES'ta DEĞİL bilinçli olarak: o liste user_id'si SONRADAN eklenen eski tablolar
   ve owner-bootstrap'taki sahipsiz satır devri içindir; bu tablo auth'tan sonra doğdu, sahipsiz
   satırı olamaz, bu yüzden user_id doğrudan NOT NULL tanımlanır. */
...
CREATE INDEX IF NOT EXISTS ai_actions_user_plan ON ai_actions (user_id, plan_id);

/* ————— Faz 34: sohbet sunucuya taşındı —————
   Mesajlar localStorage'daydı: cihaza bağlıydı (telefondan "Paylaş → Finans" ile gelen SMS
   masaüstünden görünmüyordu) ve PWA yeniden kurulunca uçuyordu. Asıl gerekçe bu DEĞİL ama:
   plan da hiçbir yerde saklanmıyordu, bu iki gerçek kusur doğuruyordu —
     1) POST /ai/execute uygulanacak işlemleri İSTEMCİDEN alıyordu; sunucu yalnız plan
        kimliğini doğruluyordu, argümanları değil. Onay kartında görünen ile uygulanan
        ayrışabilirdi, yani "gördüğünü onaylıyorsun" güvencesini istemci veriyordu.
     2) Tek kullanımlılık süreç içi bir Map'teydi; Render ücretsiz katmanı uyuyup yeniden
        başlayınca kilit buharlaşıyor ve aynı plan İKİNCİ KEZ uygulanabiliyordu (tam da
        engellemek için yazılmış çift kayıt senaryosu).
   Artık plan ai_plans'ta durur: onaylanan ile uygulanan tanım olarak aynıdır ve tek
   kullanımlılık tek bir atomik UPDATE ... WHERE consumed_at IS NULL RETURNING'dir.
   TENANT_TABLES'ta değiller (ai_actions ile aynı gerekçe: auth'tan sonra doğdular). */
...
CREATE INDEX IF NOT EXISTS ai_conversations_user ON ai_conversations (user_id, id DESC);
CREATE TABLE IF NOT EXISTS ai_messages (
  ...
  created_at text NOT NULL,
  /* doluysa bu mesaj bir UYGULAMA SONUCUdur: "geri al" düğmesi balonun altında,
     olayın geçtiği yerde durur (eskiden sohbetin dışında ayrı bir listedeydi) */
  plan_id text
);
CREATE INDEX IF NOT EXISTS ai_messages_conv ON ai_messages (conversation_id, id);
/* Onay bekleyen plan. actions = kullanıcıya GÖSTERİLEN listenin kendisi (özet + argümanlar):
   execute artık yalnız plan kimliği alır, gövdeye bakmaz. */
...
CREATE INDEX IF NOT EXISTS ai_plans_user_conv ON ai_plans (user_id, conversation_id);
-- uygulanan işlem artık hangi sohbette yapıldığını bilir (eski satırlar için NULL — aşağıda arşivlenir)
ALTER TABLE ai_actions ADD COLUMN IF NOT EXISTS conversation_id integer REFERENCES ai_conversations(id) ON DELETE SET NULL;
...
`);
  ...
  /* Faz 9 — in-place migration: recurring.amount hâlâ varsa (eski DB) tutarları zaman çizelgesine
     taşı ve kolonu düşür. Çoklu-statement simple query = tek örtük tx (backfill + drop atomik);
     ON CONFLICT DO NOTHING yarım kalmış önceki denemeye karşı güvenlik. Sonraki boot'larda no-op.
     user_id ALTER'ları yukarıda bittiğinden kolon burada kesin var. */
  const legacyAmount = await pool.query(
    "SELECT 1 FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'recurring' AND column_name = 'amount'",
  );
  if (legacyAmount.rowCount) {
    await pool.query(`
INSERT INTO recurring_amounts (recurring_id, from_month, amount, user_id)
  SELECT id, '0000-01', amount, user_id FROM recurring
ON CONFLICT (recurring_id, from_month) DO NOTHING;
ALTER TABLE recurring DROP COLUMN amount;
`);
  }
...
`);
  ...
  /* Faz 34 — sohbet öncesi uygulanmış işlemleri ARŞİV sohbetine taşı. Geri alma artık
     konuşmanın İÇİNDE yaşıyor (sonuç mesajının altındaki düğme); bağsız kalan eski satırlara
     hiçbir ekrandan ulaşılamazdı, yani asistanın var olma sebeplerinden biri olan
     "geri alabilirdin ama göremedin" durumu geri gelirdi. Kullanıcı başına tek arşiv açılır
     ve her plan için sonuç mesajı ai_actions'tan yeniden kurulur — böylece arşiv de sıradan
     bir konuşma gibi görünür, arayüzde "mesajsız plan" diye bir kenar durum doğmaz.

     BAYRAKLA bir kez çalışır, "bağsız satır kaldı mı" ölçütüyle değil: `ai_actions`in
     conversation_id'si sohbet silinince NULL'a döner (ON DELETE SET NULL — günlük audit
  ...
  if (!archived.rowCount) {
    ...
    for (const { user_id } of orphans.rows) {
      const conv = await pool.query<{ id: number }>(
        "INSERT INTO ai_conversations (user_id, title, created_at, updated_at) VALUES ($1,$2,$3,$3) RETURNING id",
        [user_id, "Önceki işlemler (arşiv)", nowLocal()],
      );
      const cid = conv.rows[0].id;
      await pool.query("UPDATE ai_actions SET conversation_id=$1 WHERE user_id=$2 AND conversation_id IS NULL", [cid, user_id]);
      await pool.query(
        `INSERT INTO ai_messages (user_id, conversation_id, role, content, created_at, plan_id)
           SELECT user_id, $1, 'assistant', string_agg('✓ ' || summary, chr(10) ORDER BY id), MIN(created_at), plan_id
             FROM ai_actions WHERE user_id=$2 AND conversation_id=$1
            GROUP BY user_id, plan_id ORDER BY MIN(id)`,
        [cid, user_id],
      );
    }
  ...
  /* Faz 15 — hareket defterini mevcut veriden bir kez doldur. Defterin kuralı balance = Σ entries
     ...
     mevduat anaparaları) yazılır ve kalan fark hesabın **açılış bakiyesi** satırına konur — böylece
     hiçbir bakiye değişmeden değişmez kural sağlanır. Bir kez çalışır (settings bayrağı); dolum
     yapılmazsa eski kayıtların silinmesi bakiyeyi kaydırırdı (geri alma deftere bakıyor). */
  const done = await pool.query("SELECT 1 FROM settings WHERE key='account_entries_backfilled'");
  if (!done.rowCount) {
    const now = nowLocal(), today = todayLocal();
    await pool.query(`
INSERT INTO account_entries (account_id, date, amount, kind, source_table, source_id, note, created_at, user_id)
  SELECT t.account_id, t.date, t.amount, 'islem', 'transactions', t.id, t.name, '${now}', t.user_id
    FROM transactions t WHERE t.account_id IS NOT NULL;
INSERT INTO account_entries (account_id, date, amount, kind, source_table, source_id, note, created_at, user_id)
  SELECT tr.account_id, tr.date,
         CASE WHEN tr.side = 'SATIŞ' THEN tr.qty * tr.price - COALESCE(tr.fee, 0)
              ELSE -(tr.qty * tr.price + COALESCE(tr.fee, 0)) END,
         'portfoy', 'trades', tr.id, tr.symbol || ' ' || tr.side, '${now}', tr.user_id
    FROM trades tr WHERE tr.account_id IS NOT NULL AND COALESCE(tr.currency, 'TRY') = 'TRY';
INSERT INTO account_entries (account_id, date, amount, kind, source_table, source_id, note, created_at, user_id)
  SELECT d.account_id, d.open_date, -d.principal, 'mevduat', 'deposits', d.id, d.name, '${now}', d.user_id
    FROM deposits d WHERE d.account_id IS NOT NULL;
INSERT INTO account_entries (account_id, date, amount, kind, source_table, source_id, note, created_at, user_id)
  SELECT a.id,
         COALESCE((SELECT MIN(e.date) FROM account_entries e WHERE e.account_id = a.id), '${today}'),
         a.balance - COALESCE((SELECT SUM(e.amount) FROM account_entries e WHERE e.account_id = a.id), 0),
         'acilis', NULL, NULL, 'Açılış bakiyesi', '${now}', a.user_id
    FROM accounts a
   WHERE a.balance - COALESCE((SELECT SUM(e.amount) FROM account_entries e WHERE e.account_id = a.id), 0) <> 0;
INSERT INTO settings (key, value) VALUES ('account_entries_backfilled', '${today}') ON CONFLICT (key) DO NOTHING;
`);
  }
}
